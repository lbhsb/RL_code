...
class DQN():
    ...
    def train_Q_network(self):
        ...
        reward_batch = [data[self.state_dim + self.action_dim] for data in minibatch]
        next_state_batch = minibatch[:, self.state_dim + self.action_dim+1:self.state_dim + self.action_dim+self.state_dim+1]
        # Step 2: calculate y
        y_batch = []
...
def main(learn_step=None):
    ...
    for episode in range(EPISODE):
        ...
        writer1.add_scalar('episode reward', episode_reward, global_step=episode)
        if episode % 10 == 0:
            print('Ep {}\tMoving average score: {:.2f}\t'.format(episode, episode_reward))
